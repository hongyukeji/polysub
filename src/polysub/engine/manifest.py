...
MODELS: Dict[str, Model] = {m.id: m for m in (
    ...
    Model("mt-8b", "mt", "Qwen3 8B（Q4_K_M）", "Qwen/Qwen3-8B-GGUF", "Qwen3-8B-Q4_K_M.gguf",
          5_027_783_488, "Apache-2.0", "d98cdcbd03e17ce47681435b5150e34c1417f50b5c0019dd560e4882c5745785"),
...
# Measured on a public 11-minute Japanese interview (docs/plans/builtin-engine.md): 30B-A3B translates best
# and fastest (a mixture of experts, ~3B active); 8B is accurate but slower and fits 16 GB; 4B fits 8 GB
# (one server at a time) but misreads more.
TIERS: Dict[str, Tuple[str, str]] = {
    "light": ("asr-qwen3", "mt-4b"),
    "standard": ("asr-qwen3", "mt-8b"),
    "high": ("asr-qwen3", "mt-30b-a3b"),
}
TIER_LABELS = {"light": "轻量（8 GB 内存）", "standard": "标准（16–24 GB 内存）", "high": "高质量（32 GB 及以上，推荐）"}
