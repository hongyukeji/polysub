...
4. **已有用户升级**（2026-09-26 负责人改定：所有人默认内置引擎）：旧配置第一次载入时迁移一次，原来的识别 / 翻译组合移到「我的模型」，任务页一键切回；已在本机用负责人的配置验证。
5. **新界面走一遍**（真机外观）：侧栏（彩色图标）、任务页顶部的添加 / 暂停按钮、深浅色切换、设置页两层、「模型」页、首次引导、任务右键「用其他质量重新翻译」、字幕编辑器 ⌘F / ⌘S、⌘⌫ 只在任务列表里生效。重拍 README 截图 `docs/images/tasks-zh-CN.png`（用公开授权视频，不用私人样片）。
