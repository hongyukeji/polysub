...
状态：**暂缓**（2026-09-25 制定；同日负责人决定先不开发，恢复时从 W0 开始）。W2 的内置识别已并入[内置引擎计划](builtin-engine.md)，恢复前先看该计划「对 Windows 计划的影响」。开发由 AI 代理执行，开工前先读文末「给执行代理的说明」；「待定事项」中与当前阶段相关的项要先由项目负责人确认。
