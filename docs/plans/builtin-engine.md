...
状态：**草案**（2026-09-25 制定，待负责人确认「待定事项」后开工）。开发由 AI 代理执行，开工前先读 [Windows 版计划](windows.md) 末尾「给执行代理的说明」，仓库约定相同。
...
- 用公开授权的测试素材（多语言，含日语对白），对比现有 oMLX Qwen3-ASR + 大模型基线：
  ...
  - 翻译：2–3 个 1.7B–8B 级候选（含专门的翻译模型），看 JSON 格式成功率（现有 `_parse` 要求一次返回整批）、人名一致性、速度。
...
1. **默认档位与质量底线**：接受默认质量低于现在的 oMLX 高配方案、换取开箱即用和速度吗？标准档用 4B 级还是 8B 级（8B 更好但 16 GB 机器会吃紧、速度约减半）？R0 数据出来后再最终确认。
